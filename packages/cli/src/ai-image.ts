import type { OgLayer, OgProject, TextLayer } from "@graphforge/core";
...
export function buildAgentImagePrompt(input: AgentImagePromptInput): string {
  ...
  return [
    "You are Codex, Claude, or OpenCode acting as the coding agent for GraphForge OG Studio.",
    "Create or author a polished 1200x630 Open Graph image for a software product.",
    "You may generate a flat bitmap, author SVG/HTML, or produce editable GraphForge JSON when that better preserves editing control.",
    ...
    input.briefPrompt ? `GraphForge generation brief:\n${input.briefPrompt}` : undefined,
    ...
    "After creating the asset, open or return it to GraphForge Studio for human review, platform preview, compression, export, and metadata publishing."
  ]
    .filter(Boolean)
    .join("\n");
}
...
export function createAgentImagePlan(input: AgentImagePlanInput): AgentImagePlan {
  ...
  return {
    ...
    instructions: [
      ...
      "Prefer editable GraphForge project JSON when the requested design can stay layered.",
      "For pure-image work, write the generated bitmap/SVG/HTML-derived export to the output path.",
      "Open GraphForge Studio after generation so the user can inspect, edit, preview platforms, compress, export, and publish metadata."
    ],
    acceptanceCriteria: [
      ...
      "No provider API key is required by GraphForge Studio or the CLI.",
      "The asset can be reviewed and exported from GraphForge Studio before metadata is applied."
    ],
    studioReview: [
      "Run graphforge studio to open the editor.",
      ...
      "Export optimized PNG/WebP/JPEG and run graphforge publish --preview before any metadata mutation."
    ]
  };
}
